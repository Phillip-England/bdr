"""Seed document generation for LLM-oriented project guidance."""
...
_SEED_VERSION_RE = re.compile(
    r"(?mi)^<!--\s*bdr-cli-version:\s*([^\s]+)\s*-->\s*$"
)


def extract_seed_version(text: str) -> str | None:
    """Return the embedded bdr CLI version from a seed file, if present."""
    m = _SEED_VERSION_RE.search(text)
    return m.group(1) if m else None


def render_seed(cli_version: str) -> str:
    """Render the canonical seed file content for the given CLI version."""
    return f"""\
<!-- bdr-seed-format: 1 -->
<!-- bdr-cli-version: {cli_version} -->
